...
* 切片：  <字符串>[M:N]    从M到N个，不包括N代表的字符。`TempStr[0:-1]`除去最后一个字符以外的字符
...
* 海龟坐标系
  * 初始位于窗口中间，朝X正向
  * `turtleforward(d)`别名`turtle.fd(d)`向前d个像素，d可以为负数
  * `turtlebackward(d)`别名`turtle.bk(d)`向后d个像素，d可以为负数
  * `turtle.circle(r,angle)`画弧线，默认圆心在海龟左侧，默认angle为360^o^
* 角度坐标
  * 极坐标方向
...
* 取值范围：[0-255]、[0-1]
* `turtle.colormode(mode)`其中`mode`可以为RGB小数或者RGB整数
...
* `turtle.pencolor(color)`其中`color`为颜色字符串或者RGB值
...
* 高级切片：指定步长切片 `<字符串>[M:N:K]` M缺失表示至开头，N缺失表示至结尾，K表示根据步长K切片
...
* `hex(x)` 或 `oct(x)` 将整数x的十六进制或者八进制写成字符串
...
* `str.replace(old, new)` 返回`str`副本，所有old的子串被替换为new
